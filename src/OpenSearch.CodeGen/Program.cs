...
using OpenSearch.CodeGen.Validation;
...
try
{
	...
	var validationPassed = CodegenValidator.Validate(allResults, sharedTypeMapper);
	if (!validationPassed)
	{
		Console.Error.WriteLine("Codegen validation failed — critical types are below minimum thresholds.");
		Environment.Exit(1);
	}
