// <auto-generated/>
#nullable enable

using System.Text.Json.Serialization;
using OpenSearch.Net;

namespace OpenSearch.Client;


/// <summary>
/// Returns a list of tasks.
/// </summary>

public sealed class ListTaskRequest
{
	/// <summary>A comma-separated list of actions that should be returned. Keep empty to return all.</summary>
	[JsonIgnore]
	public List<string>? Actions { get; set; }
	/// <summary>When `true`, the response includes detailed information about shard recoveries.</summary>
	[JsonIgnore]
	public bool? Detailed { get; set; }
	/// <summary>Groups tasks by parent/child relationships or nodes.</summary>
	[JsonIgnore]
	public GroupBy? GroupBy { get; set; }
	/// <summary>A comma-separated list of node IDs or names used to limit the returned information. Use `_local` to return information from the node you're connecting to, specify the node name to get information from a specific node, or keep the parameter empty to get information from all nodes.</summary>
	[JsonIgnore]
	public List<string>? Nodes { get; set; }
	/// <summary>Returns tasks with a specified parent task ID (`node_id:task_number`). Keep empty or set to -1 to return all.</summary>
	[JsonIgnore]
	public string? ParentTaskId { get; set; }
	/// <summary>The amount of time to wait for a response.</summary>
	[JsonIgnore]
	public string? Timeout { get; set; }
	/// <summary>Waits for the matching task to complete. When `true`, the request is blocked until the task has completed.</summary>
	[JsonIgnore]
	public bool? WaitForCompletion { get; set; }
}
public sealed class ListTaskEndpoint : IEndpoint<ListTaskRequest, ListTaskResponse>
{
	public static readonly ListTaskEndpoint Instance = new();

	public OpenSearch.Net.HttpMethod Method(ListTaskRequest r) => OpenSearch.Net.HttpMethod.Get;

	public string RequestUrl(ListTaskRequest r)
	{
		var path = $"/_tasks";
		var queryParts = new List<string>();
		if (r.Actions is not null)
			queryParts.Add($"actions={Uri.EscapeDataString(string.Join(",", r.Actions!))}");
		if (r.Detailed is not null)
			queryParts.Add($"detailed={Uri.EscapeDataString((r.Detailed.Value ? "true" : "false"))}");
		if (r.GroupBy is not null)
			queryParts.Add($"group_by={Uri.EscapeDataString(QueryParamSerializer.Serialize(r.GroupBy!.Value))}");
		if (r.Nodes is not null)
			queryParts.Add($"nodes={Uri.EscapeDataString(string.Join(",", r.Nodes!))}");
		if (r.ParentTaskId is not null)
			queryParts.Add($"parent_task_id={Uri.EscapeDataString(r.ParentTaskId!)}");
		if (r.Timeout is not null)
			queryParts.Add($"timeout={Uri.EscapeDataString(r.Timeout!)}");
		if (r.WaitForCompletion is not null)
			queryParts.Add($"wait_for_completion={Uri.EscapeDataString((r.WaitForCompletion.Value ? "true" : "false"))}");
		return queryParts.Count > 0 ? $"{path}?{string.Join("&", queryParts)}" : path;
	}

	public RequestBody? GetBody(ListTaskRequest r) => null;

	public ListTaskResponse DeserializeResponse(int statusCode, string? contentType, Stream body, IOpenSearchSerializer serializer) =>
		serializer.Deserialize<ListTaskResponse>(body)!;
}
