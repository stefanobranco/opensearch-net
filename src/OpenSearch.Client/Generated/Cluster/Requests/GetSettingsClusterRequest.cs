// <auto-generated/>
#nullable enable

using System.Text.Json.Serialization;
using OpenSearch.Net;

namespace OpenSearch.Client.Cluster;


/// <summary>
/// Returns cluster settings.
/// </summary>

public sealed class GetSettingsClusterRequest
{
	/// <summary>The amount of time to wait for a response from the cluster manager node. For more information about supported time units, see [Common parameters](https://opensearch.org/docs/latest/api-reference/common-parameters/#time-units).</summary>
	[JsonIgnore]
	public string? ClusterManagerTimeout { get; set; }
	/// <summary>Whether to return settings in the flat form, which can improve readability, especially for heavily nested settings. For example, the flat form of `"cluster": { "max_shards_per_node": 500 }` is `"cluster.max_shards_per_node": "500"`.</summary>
	[JsonIgnore]
	public bool? FlatSettings { get; set; }
	/// <summary>When `true`, returns default cluster settings from the local node.</summary>
	[JsonIgnore]
	public bool? IncludeDefaults { get; set; }
	/// <summary></summary>
	[JsonIgnore]
	public string? Timeout { get; set; }
}
public sealed class GetSettingsClusterEndpoint : IEndpoint<GetSettingsClusterRequest, GetSettingsClusterResponse>
{
	public static readonly GetSettingsClusterEndpoint Instance = new();

	public OpenSearch.Net.HttpMethod Method(GetSettingsClusterRequest r) => OpenSearch.Net.HttpMethod.Get;

	public string RequestUrl(GetSettingsClusterRequest r)
	{
		var path = $"/_cluster/settings";
		var queryParts = new List<string>();
		if (r.ClusterManagerTimeout is not null)
			queryParts.Add($"cluster_manager_timeout={Uri.EscapeDataString(r.ClusterManagerTimeout!)}");
		if (r.FlatSettings is not null)
			queryParts.Add($"flat_settings={Uri.EscapeDataString((r.FlatSettings.Value ? "true" : "false"))}");
		if (r.IncludeDefaults is not null)
			queryParts.Add($"include_defaults={Uri.EscapeDataString((r.IncludeDefaults.Value ? "true" : "false"))}");
		if (r.Timeout is not null)
			queryParts.Add($"timeout={Uri.EscapeDataString(r.Timeout!)}");
		return queryParts.Count > 0 ? $"{path}?{string.Join("&", queryParts)}" : path;
	}

	public RequestBody? GetBody(GetSettingsClusterRequest r) => null;

	public GetSettingsClusterResponse DeserializeResponse(int statusCode, string? contentType, Stream body, IOpenSearchSerializer serializer) =>
		serializer.Deserialize<GetSettingsClusterResponse>(body)!;
}
