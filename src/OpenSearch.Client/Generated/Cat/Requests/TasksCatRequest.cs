// <auto-generated/>
#nullable enable

using System.Text.Json.Serialization;
using OpenSearch.Net;

namespace OpenSearch.Client.Cat;


/// <summary>
/// Lists the progress of all tasks currently running on the cluster.
/// </summary>

public sealed class TasksCatRequest
{
	/// <summary>The task action names used to limit the response.</summary>
	[JsonIgnore]
	public List<string>? Actions { get; set; }
	/// <summary>If `true`, the response includes detailed information about shard recoveries.</summary>
	[JsonIgnore]
	public bool? Detailed { get; set; }
	/// <summary>A short version of the `Accept` header, such as `json` or `yaml`.</summary>
	[JsonIgnore]
	public string? Format { get; set; }
	/// <summary>A comma-separated list of column names to display.</summary>
	[JsonIgnore]
	public List<string>? H { get; set; }
	/// <summary>Returns help information.</summary>
	[JsonIgnore]
	public bool? Help { get; set; }
	/// <summary>A comma-separated list of node IDs or names used to limit the returned information. Use `_local` to return information from the node to which you're connecting, specify a specific node from which to get information, or keep the parameter empty to get information from all nodes.</summary>
	[JsonIgnore]
	public List<string>? Nodes { get; set; }
	/// <summary>The parent task identifier, which is used to limit the response.</summary>
	[JsonIgnore]
	public string? ParentTaskId { get; set; }
	/// <summary>A comma-separated list of column names or column aliases to sort by.</summary>
	[JsonIgnore]
	public List<string>? S { get; set; }
	/// <summary>Specifies the time units, for example, `5d` or `7h`. For more information, see [Supported units](https://opensearch.org/docs/latest/api-reference/units/).</summary>
	[JsonIgnore]
	public string? Time { get; set; }
	/// <summary>Enables verbose mode, which displays column headers.</summary>
	[JsonIgnore]
	public bool? V { get; set; }
}
public sealed class TasksCatEndpoint : IEndpoint<TasksCatRequest, TasksCatResponse>
{
	public static readonly TasksCatEndpoint Instance = new();

	public OpenSearch.Net.HttpMethod Method(TasksCatRequest r) => OpenSearch.Net.HttpMethod.Get;

	public string RequestUrl(TasksCatRequest r)
	{
		var path = $"/_cat/tasks";
		var queryParts = new List<string>();
		if (r.Format is null)
			queryParts.Add("format=json");
		if (r.Actions is not null)
			queryParts.Add($"actions={Uri.EscapeDataString(string.Join(",", r.Actions!))}");
		if (r.Detailed is not null)
			queryParts.Add($"detailed={Uri.EscapeDataString((r.Detailed.Value ? "true" : "false"))}");
		if (r.Format is not null)
			queryParts.Add($"format={Uri.EscapeDataString(r.Format!)}");
		if (r.H is not null)
			queryParts.Add($"h={Uri.EscapeDataString(string.Join(",", r.H!))}");
		if (r.Help is not null)
			queryParts.Add($"help={Uri.EscapeDataString((r.Help.Value ? "true" : "false"))}");
		if (r.Nodes is not null)
			queryParts.Add($"nodes={Uri.EscapeDataString(string.Join(",", r.Nodes!))}");
		if (r.ParentTaskId is not null)
			queryParts.Add($"parent_task_id={Uri.EscapeDataString(r.ParentTaskId!)}");
		if (r.S is not null)
			queryParts.Add($"s={Uri.EscapeDataString(string.Join(",", r.S!))}");
		if (r.Time is not null)
			queryParts.Add($"time={Uri.EscapeDataString(r.Time!)}");
		if (r.V is not null)
			queryParts.Add($"v={Uri.EscapeDataString((r.V.Value ? "true" : "false"))}");
		return queryParts.Count > 0 ? $"{path}?{string.Join("&", queryParts)}" : path;
	}

	public RequestBody? GetBody(TasksCatRequest r) => null;

	public TasksCatResponse DeserializeResponse(int statusCode, string? contentType, Stream body, IOpenSearchSerializer serializer) =>
		new();
}
