// <auto-generated/>
#nullable enable

using System.Text.Json.Serialization;
using OpenSearch.Net;

namespace OpenSearch.Client.Cat;


/// <summary>
/// Returns a concise representation of the cluster health.
/// </summary>

public sealed class HealthCatRequest
{
	/// <summary>A short version of the `Accept` header, such as `json` or `yaml`.</summary>
	[JsonIgnore]
	public string? Format { get; set; }
	/// <summary>A comma-separated list of column names to display.</summary>
	[JsonIgnore]
	public List<string>? H { get; set; }
	/// <summary>Returns help information.</summary>
	[JsonIgnore]
	public bool? Help { get; set; }
	/// <summary>A comma-separated list of column names or column aliases to sort by.</summary>
	[JsonIgnore]
	public List<string>? S { get; set; }
	/// <summary>The unit used to display time values.</summary>
	[JsonIgnore]
	public string? Time { get; set; }
	/// <summary>When `true`, returns `HH:MM:SS` and Unix epoch timestamps.</summary>
	[JsonIgnore]
	public bool? Ts { get; set; }
	/// <summary>Enables verbose mode, which displays column headers.</summary>
	[JsonIgnore]
	public bool? V { get; set; }
}
public sealed class HealthCatEndpoint : IEndpoint<HealthCatRequest, HealthCatResponse>
{
	public static readonly HealthCatEndpoint Instance = new();

	public OpenSearch.Net.HttpMethod Method(HealthCatRequest r) => OpenSearch.Net.HttpMethod.Get;

	public string RequestUrl(HealthCatRequest r)
	{
		var path = $"/_cat/health";
		var queryParts = new List<string>();
		if (r.Format is null)
			queryParts.Add("format=json");
		if (r.Format is not null)
			queryParts.Add($"format={Uri.EscapeDataString(r.Format!)}");
		if (r.H is not null)
			queryParts.Add($"h={Uri.EscapeDataString(string.Join(",", r.H!))}");
		if (r.Help is not null)
			queryParts.Add($"help={Uri.EscapeDataString((r.Help.Value ? "true" : "false"))}");
		if (r.S is not null)
			queryParts.Add($"s={Uri.EscapeDataString(string.Join(",", r.S!))}");
		if (r.Time is not null)
			queryParts.Add($"time={Uri.EscapeDataString(r.Time!)}");
		if (r.Ts is not null)
			queryParts.Add($"ts={Uri.EscapeDataString((r.Ts.Value ? "true" : "false"))}");
		if (r.V is not null)
			queryParts.Add($"v={Uri.EscapeDataString((r.V.Value ? "true" : "false"))}");
		return queryParts.Count > 0 ? $"{path}?{string.Join("&", queryParts)}" : path;
	}

	public RequestBody? GetBody(HealthCatRequest r) => null;

	public HealthCatResponse DeserializeResponse(int statusCode, string? contentType, Stream body, IOpenSearchSerializer serializer) =>
		new();
}
