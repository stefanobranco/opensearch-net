// <auto-generated/>
#nullable enable

using System.Text.Json.Serialization;
using OpenSearch.Net;

namespace OpenSearch.Client.Cat;


/// <summary>
/// Returns cluster-wide thread pool statistics per node. By default the active, queued, and rejected statistics are returned for all thread pools.
/// </summary>

public sealed class ThreadPoolCatRequest
{
	/// <summary>A comma-separated list of thread pool names used to limit the request. Accepts wildcard expressions.</summary>
	[JsonIgnore]
	public List<string>? ThreadPoolPatterns { get; set; }
	/// <summary>A timeout for connection to the cluster manager node.</summary>
	[JsonIgnore]
	public string? ClusterManagerTimeout { get; set; }
	/// <summary>A short version of the `Accept` header, such as `json` or `yaml`.</summary>
	[JsonIgnore]
	public string? Format { get; set; }
	/// <summary>A comma-separated list of column names to display.</summary>
	[JsonIgnore]
	public List<string>? H { get; set; }
	/// <summary>Returns help information.</summary>
	[JsonIgnore]
	public bool? Help { get; set; }
	/// <summary>Returns local information but does not retrieve the state from the cluster manager node.</summary>
	[JsonIgnore]
	public bool? Local { get; set; }
	/// <summary>A comma-separated list of column names or column aliases to sort by.</summary>
	[JsonIgnore]
	public List<string>? S { get; set; }
	/// <summary>The multiplier in which to display values.</summary>
	[JsonIgnore]
	public int? Size { get; set; }
	/// <summary>Enables verbose mode, which displays column headers.</summary>
	[JsonIgnore]
	public bool? V { get; set; }
}
public sealed class ThreadPoolCatEndpoint : IEndpoint<ThreadPoolCatRequest, ThreadPoolCatResponse>
{
	public static readonly ThreadPoolCatEndpoint Instance = new();

	public OpenSearch.Net.HttpMethod Method(ThreadPoolCatRequest r) => OpenSearch.Net.HttpMethod.Get;

	public string RequestUrl(ThreadPoolCatRequest r)
	{
		var path = r.ThreadPoolPatterns is not null
			? $"/_cat/thread_pool/{Uri.EscapeDataString(r.ThreadPoolPatterns!.ToString()!)}" : true
			? $"/_cat/thread_pool"
			: throw new InvalidOperationException("No valid path for the given parameters.");
		var queryParts = new List<string>();
		if (r.Format is null)
			queryParts.Add("format=json");
		if (r.ClusterManagerTimeout is not null)
			queryParts.Add($"cluster_manager_timeout={Uri.EscapeDataString(r.ClusterManagerTimeout!)}");
		if (r.Format is not null)
			queryParts.Add($"format={Uri.EscapeDataString(r.Format!)}");
		if (r.H is not null)
			queryParts.Add($"h={Uri.EscapeDataString(string.Join(",", r.H!))}");
		if (r.Help is not null)
			queryParts.Add($"help={Uri.EscapeDataString((r.Help.Value ? "true" : "false"))}");
		if (r.Local is not null)
			queryParts.Add($"local={Uri.EscapeDataString((r.Local.Value ? "true" : "false"))}");
		if (r.S is not null)
			queryParts.Add($"s={Uri.EscapeDataString(string.Join(",", r.S!))}");
		if (r.Size is not null)
			queryParts.Add($"size={Uri.EscapeDataString(r.Size.ToString()!)}");
		if (r.V is not null)
			queryParts.Add($"v={Uri.EscapeDataString((r.V.Value ? "true" : "false"))}");
		return queryParts.Count > 0 ? $"{path}?{string.Join("&", queryParts)}" : path;
	}

	public RequestBody? GetBody(ThreadPoolCatRequest r) => null;

	public ThreadPoolCatResponse DeserializeResponse(int statusCode, string? contentType, Stream body, IOpenSearchSerializer serializer) =>
		new();
}
